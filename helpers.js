...
const measureWidthCtx = function (text, font, wordSpacing) { // scale = 1
    if (typeof font === 'string') measureCtx.font = font;
    let wsPx;
    if (!wordSpacing) {
        wsPx = 0;
    } else {
        if (typeof wordSpacing === 'number') {
            wsPx = wordSpacing * initialMetrics.fontSize;
        } else if (typeof wordSpacing === 'string') {
            if (/px/.test(wordSpacing)) {
                wsPx = parseFloat(wordSpacing.replace("px", "").trim());
            } else if (/em/.test(wordSpacing)) {
                wsPx = parseFloat(wordSpacing.replace("em", "").trim()) * initialMetrics.fontSize;
            } else {
                wsPx = 0
...
const getLineWidth = function (line, wordSpacing = undefined) {
    ...
    let width = !contentSpan ? 0 : contentSpan.getBoundingClientRect().width / (typeof scaleRatio === 'number' ? scaleRatio : 1);
    if (wordSpacing) lineEle.style.wordSpacing = currentSpacing; // reset ws
    return width;
}

/*
    get line width after a subsitution, return the minWidth (with min word space) and maxWidth (with max word space)
...
/*
return the est change of width in percentage after a word change
{
...
@param: newWord: str, the word  to change to
        ...
        field: arr, return field, ['max', 'min', 'opt'] // only get the neccessary ones for the best performance
*/
const estWidthChangePercentage = function (newWord, wordIdx, field = ['max', 'min']) {
    let r = {};
    ...
    if (field.includes('max')) {
        ...
        r.max = [((widthMaxWs - targetWidth)/targetWidth)*100, widthMaxWs]
    }
    if (field.includes('min')) {
        ...
        r.min = [((widthMinWs - targetWidth)/targetWidth)*100, widthMinWs]
    }
    if (field.includes('opt')) {
        let currentWsPx = parseFloat(style.wordSpacing.replace("px", "").trim())
        let numOfSpace = newtxt.split(" ").length - 1;
        ...
        r.opt = [((finalWidth - targetWidth)/finalWidth)*100, finalWidth, finalWs]
    }
    return r
}

/*
return the change of width in percentage after a word change using client rectangle
{
    min: array[percentage, width] distance to target width with minimal word spacing,
    max: array[percentage, width] distance to target width with max word spacing
    opt: array[percentage, width, ws(num in px)] distance to target width after ws adjustment
...
@param: newWord: str, the word  to change to
        wordId: int, the id of the word to be changed
        field: arr, return field, ['max', 'min', 'opt'] // only get the neccessary ones for the best performance but this is generally slower than estWidthChangePercentage
*/
const widthChangePercentage = function (newWord, wordIdx, field = ['max', 'min']) {
    let r = {};
    ...
    if (field.includes('opt')) {
        ...
        r.opt = [((finalWidth - targetWidth)/targetWidth)*100, finalWidth, finalWs]
    }
    wordEle.textContent = originalWord;
    return r
}
