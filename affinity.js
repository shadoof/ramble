...
let lines = circleLayout(sources.rural, cx, cy, radius, 21.9, fontFamily, 30);
//console.log(lines);
//let lines = maxFontSizeForCircle(sources.rural, cx, cy, radius, fontFamily, 30);
spanify(lines);
//ramble();
...
function spanify(lines) {
  let wordIdx = 0;
  domDisplay.innerHTML = lines.reduce((html, l) => {
    let words = l.text.split(' ')
    let ypos = l.bounds[1] - l.bounds[3] / 2;
    let line = words.reduce((acc, word) => {
