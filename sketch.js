...
spanify(state.current.words, display);
loop();
...
function loop() {
  ...
  if (state.updating) {
    ...
    if (!currentUpdate) console.warn('No update from:' + current.name, current.replacements());

    const shadowUpdate = shadow.step(outgoing);
    if (!shadowUpdate) console.warn('No shadow update:' + shadow.name, shadow.replacements());
...
function updateState(update) {
  ...
  const steps = current.replacements();
