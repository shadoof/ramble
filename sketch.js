...
function updateState(update) {
  ...
  const { outgoing, shadow, current, maxSteps } = state;
...
function updateDOM(next, idx) {
  ...
  //console.log('setting '+ele.innerText+' to be '+next);   
  ele.innerText = next;
  let dirty = state.current.isModified(idx);

  ele.style.backgroundColor = (dirty ? '#fbb' : '#fff');
  // "#fbb" // outgoing
  // "#bbf"; // incoming
}
